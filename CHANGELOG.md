...
- **ReplayMod compatibility** — local display video pauses/resumes with ReplayMod and is corrected to its millisecond replay playhead during rendering; pause/resume, seek, video-change, and display-GUI actions are written as persistent `.mcpr` markers and replayed on the timeline; temporary replay positions are never reported back to the live server display timeline.
