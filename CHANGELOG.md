...
- **Bilibili session refresh fix** — `BilibiliSessionRefresher` now sends a persistent `buvid3` cookie (format: `UUIDv4.toUpperCase() + random5digits + "infoc"`, matching PiliPlus) with every refresh request. Bilibili's passport API requires this device fingerprint — without it the endpoint returns `-400 请求错误`. The `buvid3` is generated once on first login and stored in the credential store alongside the SESSDATA and refresh token. Also added `Origin` header and enhanced the refresh-failure log to include the Bilibili error code for easier diagnosis.
- **Bilibili session refresh removed** — Bilibili's passport web-cookie refresh endpoint (`/x/passport-login/web/cookie/refresh`) consistently returns `-400 请求错误` even with `buvid3` in both the Cookie header and the POST form body plus the `Origin` header. The endpoint's device-fingerprint requirements are impractical to satisfy from a headless server, so the automatic refresh sweeps (`BilibiliSessionRefresher.kt`, `refreshAllBilibili()`, hourly jobs in the Paper and vanilla bootstraps) are all removed. When a stored SESSDATA expires, the user simply logs in again via `/display login` — consistent behavior with several other Bilibili clients.
