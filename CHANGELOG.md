...
- **ReplayMod compatibility** — during replay rendering, local display media is frozen and the visible frame follows the ReplayMod millisecond playhead, preventing slow export FPS from speeding up the video; replay pause holds the frame, while pause/resume, seek, video-change, and display-GUI actions are written as persistent `.mcpr` markers and replayed on the timeline. ReplayMod has no public external-PCM injection API, so display audio is not mixed into its FFmpeg output; audio bridging remains a tracked follow-up. Replay rendering now drops stale frame-seek requests and resets marker playback after timeline rewinds.
