...
Current clients negotiate the improved V3 transport when available. V3 uses a versioned, batch-capable envelope on `dreamdisplayx:v3`; clients automatically fall back to the compatible V2 envelope on `dreamdisplayx:v2`. V3 same-content snapshots can bind multiple displays to one URL and authoritative playback timeline. Legacy V1 traffic is detected and the affected player is notified in chat, but V1 packets are not processed.
