...
- Danmaku (弹幕) overlay — render Bilibili / YouTube comments scrolling over displays. Reference implementation: [squi2rel/VideoPlayer](https://github.com/squi2rel/VideoPlayer), package `com.github.squi2rel.vp.danmaku`.
  ...
  - Sources: `BiliVodDanmakuFetcher` + `BiliDmSegParser` (protobuf `/x/v2/dm/list/seg.so`), `BiliLiveDanmakuClient` (live WebSocket), `YouTubeLiveChatClient` / `YouTubeLiveChatProtocol`; reuse the existing Bilibili `SESSDATA` / WBI auth for fetching.
