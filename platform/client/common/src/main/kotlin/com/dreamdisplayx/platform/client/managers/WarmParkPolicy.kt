...
import com.dreamdisplayx.platform.client.capabilities.ClientMemoryProbe
import com.dreamdisplayx.platform.client.displays.DisplayScreen
import com.dreamdisplayx.platform.client.managers.WarmParkPolicy.MAX_AUTO_WARM_LIMIT
import com.dreamdisplayx.platform.client.managers.WarmParkPolicy.replayOnly
...
object WarmParkPolicy {
    ...
    /** Operator override for the warm limit via `dreamdisplayx.warmPark.max`, or `null` to auto-compute. */
    private val configuredMax: Int? =
        System.getProperty("dreamdisplayx.warmPark.max")?.toIntOrNull()?.coerceAtLeast(0)

    /** Whether full warm-park is disabled in favor of replay snapshots (`dreamdisplayx.warmPark.replayOnly`). */
    private val replayOnly: Boolean =
        System.getProperty("dreamdisplayx.warmPark.replayOnly")?.toBooleanStrictOrNull() ?: DEFAULT_REPLAY_ONLY
    ...
    val demoteAfterNanos: Long =
        (System.getProperty("dreamdisplayx.warmPark.demoteMs")?.toLongOrNull()?.coerceAtLeast(0L)
            ?: DEFAULT_DEMOTE_MS) * 1_000_000L

    /** Time a dormant display may sit parked before teardown, in nanoseconds. */
    val ttlNanos: Long =
        (System.getProperty("dreamdisplayx.warmPark.ttlMs")?.toLongOrNull()?.coerceAtLeast(0L)
