...
/**
 * Small popup listing the Bilibili media-type filters (all / video / bangumi / movie), opened from
 * the suggestions panel's filter button. Same visual / interaction style as [SortDropdown].
 */
class BilibiliFilterDropdown(
    private val current: () -> BilibiliSearchType,
    private val onSelect: (BilibiliSearchType) -> Unit,
) {
    var visible: Boolean = false

    private val items: List<BilibiliSearchType> = BilibiliSearchType.entries.toList()

    private var rect = UiRect(0, 0, WIDTH, ITEM_H * items.size)

    /** Appear / disappear progress in 0..1, eased the same way as [SortDropdown]. */
    ...
    /** Draws the dropdown anchored below ([anchorCenterX], [anchorY]) — the filter button sits near the top of the suggestions panel. */
    fun draw(g: GuiGraphicsCompat, anchorCenterX: Int, anchorY: Int, mouseX: Int, mouseY: Int) {
        ...
        items.forEachIndexed { i, option ->
            ...
            val lbl = Component.translatable(option.labelKey).string
            g.drawText(font, UiText.trim(font, lbl, WIDTH - 10), rect.x + 6, fy + ITEM_H * i, color, false)
        }
    ...
    companion object {
        ...
        /** Tint for the currently active filter type's row. */
