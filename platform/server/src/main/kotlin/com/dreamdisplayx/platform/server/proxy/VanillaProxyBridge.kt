...
object VanillaProxyBridge {
    private val logger = LoggerFactory.getLogger("DreamDisplaysX/VanillaProxyBridge")
    ...
    /** Called from [com.dreamdisplayx.platform.server.VanillaBootstrap.onServerStarted]. Resets per-session state. */
    ...
    /** Encodes and sends [packet] on the `dreamdisplayx:proxy` channel via [player]'s connection to the proxy. */
    ...
    /** Decodes and handles one `dreamdisplayx:proxy` message that arrived on [player]'s connection. */
