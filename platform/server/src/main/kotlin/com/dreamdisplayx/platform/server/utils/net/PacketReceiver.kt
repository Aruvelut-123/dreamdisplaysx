...
class PacketReceiver(private val plugin: PaperServer) : PluginMessageListener {
    ...
    override fun onPluginMessageReceived(channel: String, player: Player, message: ByteArray) {
        when (channel) {
            "dreamdisplayx:sync" -> handleSyncPacket(player, message)
            "dreamdisplayx:req_sync" -> handleRequestSync(player, message)
            "dreamdisplayx:delete" -> handleDelete(player, message)
            "dreamdisplayx:report" -> handleReport(player, message)
            "dreamdisplayx:version" -> handleVersion(player, message)
            "dreamdisplayx:display_enabled" -> handleDisplayEnabled(player, message)
            "dreamdisplayx:set_video" -> handleSetVideo(player, message)
            "dreamdisplayx:set_locked" -> handleSetLocked(player, message)
        }
    }
