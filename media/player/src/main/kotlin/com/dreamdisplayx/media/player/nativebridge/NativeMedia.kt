...
package com.dreamdisplayx.media.player.nativebridge
...
/**
 * Java FFM bridge to optional `dreamdisplayx_native` Rust library (`FFmpeg`, NV12 -> RGB, brightness).
 */
object NativeMedia {
    /** Logger. */
    private val logger = LoggerFactory.getLogger("DreamDisplaysX/NativeMedia")
    ...
    private const val LIB_BASE_NAME = "dreamdisplayx_native"
    private const val LAV_BASE_NAME = "dreamdisplayx_lav"
    private const val LAV_SURFACE_ABI_VERSION = 1
    private const val LAV_SURFACE_DESC_BYTES = 80L
    private const val CACHE_ROOT = "./dreamdisplayx/native"
    ...
    val nv12Enabled: Boolean = System.getProperty("dreamdisplayx.native.nv12", "true").toBoolean()
    ...
    val rgbaFramesEnabled: Boolean
        get() = isAvailable
                && System.getProperty("dreamdisplayx.native.rgba", "true").toBoolean()
    ...
    val yuvGpuEnabled: Boolean
        get() = isAvailable
                && nv12Enabled
                && System.getProperty("dreamdisplayx.native.yuvgpu", "true").toBoolean()
                && videoReadFrameI420Handle != null
                && i420ToRgbaHandle != null

    /** In-process decode (libavformat / libavcodec in `dreamdisplayx_lav`). */
    val lavInProcessEnabled: Boolean
        get() = yuvGpuEnabled
                && System.getProperty("dreamdisplayx.native.libav", "true").toBoolean()
                && lavAvailable
    ...
    val lavZeroCopyEnabled: Boolean
        get() = lavInProcessEnabled
                ...
                && System.getProperty("dreamdisplayx.native.libav.zeroCopy", "false").toBoolean()

    /** True once the optional `dreamdisplayx_lav` library has been located, loaded, and bound. */
    val lavAvailable: Boolean by lazy { isAvailable && runCatching { initLav() }.getOrDefault(false) }

    /** True when `dreamdisplayx_lav` exports the additive hardware-surface ABI. */
    ...
    private fun init(): Boolean {
        if (!System.getProperty("dreamdisplayx.native", "true").toBoolean()) {
            logger.info("Native pipeline disabled via -Ddreamdisplayx.native=false.")
        ...
        return try {
            ...
            val rgba = System.getProperty("dreamdisplayx.native.rgba", "true").toBoolean()
                    && videoReadFrameRgbaHandle != null
    ...
    /**
     * Binds the optional in-process libav library (`dreamdisplayx_lav`). It links FFmpeg
    ...
    /**
     * Portable LAV bundles may place FFmpeg shared libraries next to `dreamdisplayx_lav`.
    ...
    private fun preloadLavDependencies(dir: File?) {
        ...
        val libraries =
            dir.listFiles()?.filter { it.isFile && isSharedLibrary(it.name) && !isDreamDisplaysXLibrary(it.name) }
                ?: return
    ...
    private fun isDreamDisplaysXLibrary(name: String): Boolean {
        val lower = name.lowercase()
        return lower.contains("dreamdisplayx_native") || lower.contains("dreamdisplayx_lav")
    }
    ...
    /**
     * Locates the platform library: explicit `-Ddreamdisplayx.native.path`, then the
    ...
    private fun locateLibrary(baseName: String = LIB_BASE_NAME): File? {
        val explicitPathProperty = when (baseName) {
            LIB_BASE_NAME -> "dreamdisplayx.native.path"
            LAV_BASE_NAME -> "dreamdisplayx.native.lav.path"
            else -> null
        }
        ...
        val resource = "/dreamdisplayx-natives/${platformKey()}/$libName"
