...
internal class NativeVideoFramePipe(
    ...
    private val logger = LoggerFactory.getLogger("DreamDisplaysX/NativeVideoFramePipe")
    ...
    companion object {
        ...
        private val cacheWindowMs: Long =
            System.getProperty("dreamdisplayx.cache.windowMs")?.toLongOrNull()?.coerceIn(0, 120_000)
                ?: DEFAULT_CACHE_WINDOW_MS

        /** Hard byte cap for one live LAV packet ring. */
        private val cacheMaxBytes: Long =
            System.getProperty("dreamdisplayx.cache.maxBytes")?.toLongOrNull()?.coerceAtLeast(0)
     ...
     * `-Ddreamdisplayx.native.libav.hw=<backend>` pins it for driver debugging.
     */
    private fun lavHwCode(hwAccel: HwAccelBackend): Int {
        if (hwAccel == HwAccelBackend.NONE) return HwAccelBackend.NONE.lavCode
        val configured = System.getProperty("dreamdisplayx.native.libav.hw")?.lowercase()
