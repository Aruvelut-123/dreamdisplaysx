...
object CustomMediaPolicy {
    ...
    fun evaluate(url: String, settings: Settings): Verdict {
        ...
        val host = hostOf(url) ?: return Verdict.MALFORMED
    ...
    /**
     * Host of [url]: the http(s) host, or the endpoint host of an ingest URL (`rtmp://`, `rtmps://`,
     * `srt://`) which [MediaHttpUrl] rejects by design. Ingest links are matched against the same
     * host rules as everything else, instead of being treated as malformed.
     */
    private fun hostOf(url: String): String? {
        MediaHttpUrl.parse(url)?.let { return it.uri.host?.lowercase(Locale.ROOT) }
        if (CustomMediaUrls.isIngest(url)) {
            return runCatching { java.net.URI(url.trim()).host?.lowercase(Locale.ROOT) }.getOrNull()
        }
        return null
    }
