 ...
 * The arch subdirectory is appended directly.
 */
private fun sqliteNativeSubPath(osName: String, arch: String): String? = when {
    osName.equals("Linux", ignoreCase = true) && arch == "x86_64" -> "Linux/x86_64"
    osName.equals("Linux", ignoreCase = true) && arch == "aarch64" -> "Linux/aarch64"
    osName.equals("Mac", ignoreCase = true) && arch == "x86_64" -> "Mac/x86_64"
    osName.equals("Mac", ignoreCase = true) && arch == "aarch64" -> "Mac/aarch64"
    osName.equals("Windows", ignoreCase = true) && arch == "x86_64" -> "Windows/x86_64"
    osName.equals("Windows", ignoreCase = true) && arch == "aarch64" -> "Windows/aarch64"
    else -> null
}
 ...
 *
 * The CI build-sqlite-natives job produces:
 * ```
 * <nativeBundleDir>/
...
fun ShadowJar.includeRebuiltSqliteNatives(nativeBundleDir: File) {
    ...
    nativeBundleDir.listFiles()?.filter { it.isDirectory }?.forEach { osDir ->
        ...
        osDir.listFiles()?.filter { it.isDirectory }?.forEach { archDir ->
            // archDir = "x86_64", "aarch64"
            val subPath = sqliteNativeSubPath(osDir.name, archDir.name) ?: return@forEach
            archDir.listFiles()?.filter { it.isFile }?.forEach { lib ->
